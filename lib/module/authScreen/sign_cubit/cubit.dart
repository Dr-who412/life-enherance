...
class SignCubit extends Cubit<SignState>{
  ...
  ///signup to server
  UserModel? user;
  singUp({required String email,required String name,required String password})async{
  ...
  postData(pathUrl: AppApi.Signup, body: {
    ...
    if(value['user']!=null){
      ...
      CacheHelper.saveData(key:'token',value:'${AppConstant.Token}',).then((va) {
   ...
   logIn({required String email,required String password}){
    ...
    postData(pathUrl: AppApi.Login, body: {
      ...
      AppConstant.Token=value['token'];
      CacheHelper.saveData(key:'token',value:'${AppConstant.Token}',).then((vl) {
        user=UserModel.fromJson(value['user']);
        showtoast(text: 'hi ${user?.name}', state:toastStates.SUCESS);
   ...
   getUserData(){
    getData(pathUrl: AppApi.GetUser,token: AppConstant.Token,).then((value) {
    if(value['status']==true){
      user=UserModel.fromJson(value['user']);
    showtoast(text: 'hi ${user?.name}', state:toastStates.SUCESS);
  ...
  void updateDate({required int? BMI,required int? height,required int? weight,bool isMale=true}){
